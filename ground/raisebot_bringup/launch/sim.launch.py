...
def launch_setup(context, *args, **kwargs):
    ...
    pkg_worlds = get_package_share_directory('raisebot_worlds')
    pkg_description = get_package_share_directory('raisebot_description')
    pkg_bringup = get_package_share_directory('raisebot_bringup')
    pkg_ros_gz_sim = get_package_share_directory('ros_gz_sim')

    world_path = os.path.join(pkg_worlds, 'worlds', world)
    robot_xacro = os.path.join(pkg_description, 'urdf', 'raisebot_robot.urdf.xacro')
...
def generate_launch_description():
    ...
    return LaunchDescription([
        DeclareLaunchArgument(
            ...
            description='SDF in raisebot_worlds/worlds/. Use greenhouse_2026_lite.sdf on CPU_ONLY.',
        ),
