...
```bash
# Full sim
ros2 launch agribot_bringup sim.launch.py

# CPU-only laptop — use the lite world
ros2 launch agribot_bringup sim.launch.py world:=greenhouse_2026_lite.sdf

# Spawn robot at a different aisle
ros2 launch agribot_bringup sim.launch.py y:=-1.0

# World alone (no robot, no bridge — fast sanity check)
ros2 launch agribot_bringup world_only.launch.py
```
