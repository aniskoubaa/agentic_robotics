#!/usr/bin/env python3
"""
AgriBot — move_to_pose server (UR5e named poses).
...
- Each callback publishes 6 `std_msgs/Float64` targets, one per UR5e
  ...
  plugins declared in agribot_gazebo.urdf.xacro, which apply the PID.
...
# topic per joint — those are bridged in agribot_bringup/config/ros_gz_bridge.yaml.
UR5E_JOINTS = [
...
# "home" matches the URDF default in agribot_gazebo.urdf.xacro so a
