...
#
#     __import__('pkg_resources').require('agribot-labs==0.1.0')
...
# of develop, so `import agribot_labs` still works.
    ...
    def _write_simple_wrapper(self, src_path: str) -> None:
        ...
        with open(target, 'w') as f:
            f.write(
                ...
                '# AgriBot — dev-script wrapper (no pkg_resources, no DeprecationWarning).\n'
...
# Scripts still install by basename into lib/agribot_labs/, so
# `ros2 run agribot_labs <script>.py` is unaffected by the nesting.
LABS = [
...
def lab_readme_data_files():
    """Install each lab's README under share/agribot_labs/<lab>/README.md."""
    files = []
    for lab in LABS:
...
def day2_lib_data_files():
    ...
    the libs under share/agribot_labs/day2_lib/ and _d2paths falls back to
...
setup(
    ...
    description='AgriBot lab starter scripts.',
    license='MIT',
