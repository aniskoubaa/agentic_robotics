#!/usr/bin/env python3
"""
AgriBot — Lab 1, Script 08 — call the YOLO object detector.
...
║     agribot_tools/agribot_tools/detector_server.py                    ║
║                                                                           ║
    ...
    for key, (label, srv_name) in ACTIONS.items():
        if not clients[key].wait_for_service(timeout_sec=SERVER_TIMEOUT):
            node.get_logger().error(
                ...
                f'    ros2 run agribot_tools detector_server'
            )
            rclpy.shutdown()
            sys.exit(1)
