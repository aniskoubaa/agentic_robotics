...
"""
AgriBot — Lab 2.2 (bridge) — fine-tune SmolVLA on this morning's demos.
...
Usage:
    ...
    ros2 run agribot_labs finetune_smolvla.py --task A --team team07 --dry-run
...
def main():
    ap = argparse.ArgumentParser(description='AgriBot SmolVLA fine-tune launcher')
    ...
    ap.add_argument('--data-root', default=str(_d2paths.DATASETS_DIR),
                    help='where the dataset lives — inside the repo (AgriBot/datasets), must match where it was recorded')
