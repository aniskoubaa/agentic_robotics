...
"""
AgriBot — Lab 2.1, Script 05 — AUTO-DEMONSTRATOR ("pick the red tomato").
...
from agribot_tools import gz_utils   # noqa: E402
...
class Demonstrator(Node):
    ...
    def tool_point(self, grasp_offset: float = 0.13):
        ...
        try:
            ...
            return None
        wtb = gz_utils.get_model_world_pose('agribot_robot', self.world)
        if wtb is None:
...
def main():
    ap = argparse.ArgumentParser(description='AgriBot auto-demonstrator (pick the red tomato)')
    ...
    ap.add_argument('--data-root', default=str(_d2paths.DATASETS_DIR),
                    help='datasets live INSIDE the repo (AgriBot/datasets) so everything is versioned together')
