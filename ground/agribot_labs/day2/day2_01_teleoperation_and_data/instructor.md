...
4. One smoke episode: `ros2 run agribot_labs 05_* --episodes 1 --dry-run` → `grasp OK`.
5. The reference dataset is in the repo (`AgriBot/datasets/...ref/`) — show
...
| 10–25 | Path B taste: everyone drives `ros2 run agribot_labs 01_*` for 10 min (let them feel the pain) |
| 25–35 | `ros2 run agribot_labs 02_*` pre-flight: what the model actually sees |
| 35–75 | Path A: `ros2 run agribot_labs 05_* --episodes 30`; while it runs, walk through the script's WHY comments (grasp-by-decree, no-IK trick) |
| 75–90 | `ros2 run agribot_labs 06_* --spawn` replay verification — every team replays ≥1 episode |
...
- **Recorder saves 0-frame episodes** — streams not up yet; wait for `ros2 run agribot_labs 02_*` to
