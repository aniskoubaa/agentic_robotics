...
- **Graspable tomatoes** `tomato_red` / `tomato_green` (gravity off → they stay
  where spawned) — `agribot_worlds/meshes/`.
- **`grasp_server`** — a deterministic "fake grasp": when the gripper closes near
  a registered tomato, it sticks to the tool; opening releases it. `agribot_tools`.
- **World-pose bridge** `/gz_world_poses` — every model/link's WORLD pose, so the
  grasp uses one consistent frame. `agribot_bringup/config/ros_gz_bridge.yaml`.
...
## Build first
```bash
cd ~/Dev_WS/raise_summer_school/AgriBot/raise_ros2_ws
...
> model — confirm `install/agribot_worlds/share/.../meshes/tomato_red/` exists
> and you re-sourced `install/setup.bash`.

## Step 3 — The grasp attaches
```bash
ros2 run agribot_tools grasp_server         # terminal 3  (alias: grasp_d3)
...
> `ros2 run agribot_tools grasp_server -p attach_radius:=0.20`. If it grabs the
...
✅ **Done when:** episodes save under `AgriBot/datasets/me__raise_ripeness_sort_team07` (inside the repo)
