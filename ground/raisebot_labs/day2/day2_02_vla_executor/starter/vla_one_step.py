...
"""
RaiseBot — Lab 2.2 warm-up: ONE call to the VLA, with every input and the
output made visible.
...
class OneStepIO(Node):
    ...
    def tool_point(self, grasp_offset=0.13):
        ...
        try:
            ...
            return None
        wtb = gz_utils.get_model_world_pose('raisebot_robot')
        if wtb is None:
            return None
...
def spawn_scene(node, red_left=True):
    ...
    if left_pt is None or right_pt is None:
        ...
        if gz_utils.get_model_world_pose('raisebot_robot') is None:
