...
"""
RaiseBot — Lab 2.2 (bridge) — fine-tune SmolVLA on this morning's demos.
...
Usage:
    ...
    ros2 run raisebot_labs finetune_smolvla.py --task A --team team07 --dry-run

    # Launch it in the background on the GPU box (fire-and-forget):
    ros2 run raisebot_labs finetune_smolvla.py --task A --team team07 \
...
def main():
    ap = argparse.ArgumentParser(description='RaiseBot SmolVLA fine-tune launcher')
    ...
    ap.add_argument('--data-root', default=str(_d2paths.DATASETS_DIR),
                    help='where the dataset lives — inside the repo (RaiseBot/datasets), must match where it was recorded')
