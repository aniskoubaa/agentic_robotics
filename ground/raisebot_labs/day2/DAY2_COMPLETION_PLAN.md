...
**Deliverable:** a handful of episodes that **replay as clean picks** (`ros2 run raisebot_labs 06_* --spawn`).
...
**Steps:** `ros2 run raisebot_labs 05_* --episodes 40` → `agr_finetune --launch` (start `--steps 3000` to prove it, then a longer run). **Measure the wall-clock** and pin it into `DAY2_LAB_DESIGN.md §3.1` + `instructor.md`.
