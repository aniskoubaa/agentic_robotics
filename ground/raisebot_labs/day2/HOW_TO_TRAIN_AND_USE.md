...
> (a scripted expert drives). Keyboard teleop exists (`ros2 run raisebot_labs 01_*`) and is worth 10
...
> uses the **venv python** — the aliases (`ros2 run raisebot_labs 03_* ros2 run raisebot_labs 05_* ros2 run raisebot_labs 06_* agr_finetune
...
grasp_d3          # = ros2 run raisebot_tools grasp_server
```
...
```bash
# terminal 3
ros2 run raisebot_labs 05_* --episodes 50 --team team07 --hf-user me
```
...
```bash
ros2 run raisebot_labs 06_* --task C --team team07 --hf-user me --episode 0 --spawn
```
...
`RaiseBot/datasets/me__raise_ripeness_sort_team07`
(see a finished example with images: `RaiseBot/datasets/raiseschool__raise_ripeness_sort_ref/`).

<details><summary>⌨️ Manual alternative (drive it yourself)</summary>

```bash
ros2 run raisebot_labs 01_* --task C          # terminal 3 — keyboard teleop (h = help)
ros2 run raisebot_labs 03_* --task C --team team07 --hf-user me    # terminal 4 — recorder prompts you per episode
```
...
checkpoint** when present (see `RaiseBot/checkpoints/README.md`). Expected:
...
~/raise_venvs/lerobot/bin/python3 \
  src/raisebot_labs/day2/day2_02_vla_executor/evaluator/evaluate.py --task C --trials 10
```
...
ros2 run raisebot_labs 05_* --episodes 50 --team t --hf-user me                    # T3: collect (25 min)
ros2 run raisebot_labs 06_* --task C --team t --hf-user me --episode 0 --spawn     # verify by replay
...
- The dataset explained with images: `RaiseBot/datasets/raiseschool__raise_ripeness_sort_ref/README.md`
- Sim manipulation layer + the 4 tunables: [`VERIFY_MANIPULATION.md`](./VERIFY_MANIPULATION.md)
- Checkpoint distribution: `RaiseBot/checkpoints/README.md`
