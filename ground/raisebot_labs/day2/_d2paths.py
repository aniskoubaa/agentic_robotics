...
"""
RaiseBot — Day-2 path bootstrap (the ONE place import/venv plumbing lives).
...
What bootstrap() does:
  1. Finds the Day-2 library root in ANY install layout (student report 2026-07-11:
     ...
     where colcon COPIED sources instead of symlinking them):
       a. the source tree  (…/raisebot_labs/day2 — where this file lives), or
       b. the installed copy under share/raisebot_labs/day2_lib (shipped by
  ...
  3. lerobot=True: if lerobot isn't importable under THIS interpreter but the
     ...
     `ros2 run raisebot_labs 03_record.py` just works instead of dying with
     a misleading "lerobot not installed" (student report finding #3).
  4. Exposes DATASETS_DIR: the in-repo RaiseBot/datasets when discoverable,
     else ~/raise_datasets.
"""
...
def _find_day2_root() -> Path:
    ...
    # Installed-copy fallback: share/raisebot_labs/day2_lib (see setup.py).
    try:
        from ament_index_python.packages import get_package_share_directory
        candidates.append(Path(get_package_share_directory('raisebot_labs')) / 'day2_lib')
    except Exception:
        pass
    ...
    for c in candidates:
        if (c / 'api_clients' / 'vla_client' / '__init__.py').is_file():
            return c
    raise ImportError(
        'RaiseBot: could not locate the Day-2 libraries (api_clients/vla_client).\n'
        f'Searched: {[str(c) for c in candidates]}\n'
        'Fix: rebuild the workspace —\n'
        '    cd <repo>/RaiseBot/raise_ros2_ws && colcon build --symlink-install\n'
...
def _find_datasets_dir() -> Path:
    """The repo's RaiseBot/datasets when reachable, else ~/raise_datasets."""
    for start in (DAY2, Path(sys.prefix)):
        p = start
        for _ in range(7):
            cand = p / 'datasets'
            if cand.is_dir() and (p / 'raise_ros2_ws').is_dir():   # p == RaiseBot
                return cand
            p = p.parent
...
def bootstrap(lerobot: bool = False) -> None:
    ...
    if in_venv:
        sys.exit(
            'RaiseBot: the lerobot venv exists but LeRobot will not import inside it.\n'
            'Reinstall it:\n'
            '    ~/raise_venvs/lerobot/bin/python3 -m pip install --timeout 30 --retries 10 "lerobot[smolvla]"')
    sys.exit(
        'RaiseBot: this script needs LeRobot, and the lerobot venv is missing.\n'
