...
**Lectures — `RaiseBot/lectures/day2_l1_vlm_to_vla/`:**
- `RaiseBot_LectureD2L1_VLM_to_VLA.tex` (**22 frames**, builds clean) +
...
- `RaiseBot_D2_Comparative_VLA_Models.tex` (**23 frames**, builds clean) +
...
**Design docs — `RaiseBot/raise_ros2_ws/src/raisebot_labs/day2/`:**
...
`RaiseBot/raise_ros2_ws/src/raisebot_labs/day2/{day2_01_teleoperation_and_data, day2_02_vla_executor}/`.
...
- Each new starter gets an `XX_dN` **bashrc alias**; Day-2 uses `_d3` (Lab 2.1) /
  `_d4` (Lab 2.2). Add starters to `raisebot_labs/setup.py` LABS (paths already
  listed) → `colcon build`.
