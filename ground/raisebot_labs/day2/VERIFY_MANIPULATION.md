...
- **Graspable tomatoes** `tomato_red` / `tomato_green` (gravity off → they stay
  where spawned) — `raisebot_worlds/meshes/`.
- **`grasp_server`** — a deterministic "fake grasp": when the gripper closes near
  a registered tomato, it sticks to the tool; opening releases it. `raisebot_tools`.
- **World-pose bridge** `/gz_world_poses` — every model/link's WORLD pose, so the
  grasp uses one consistent frame. `raisebot_bringup/config/ros_gz_bridge.yaml`.
...
> model — confirm `install/raisebot_worlds/share/.../meshes/tomato_red/` exists
> and you re-sourced `install/setup.bash`.

## Step 3 — The grasp attaches
```bash
ros2 run raisebot_tools grasp_server         # terminal 3  (alias: grasp_d3)
...
> `ros2 run raisebot_tools grasp_server -p attach_radius:=0.20`. If it grabs the
...
```bash
ros2 run raisebot_labs 05_auto_demonstrate.py --episodes 4 --team team07 --hf-user me
# alias:  ros2 run raisebot_labs 05_* --episodes 4 --team team07 --hf-user me
```
...
✅ **Done when:** episodes save under `RaiseBot/datasets/me__raise_ripeness_sort_team07` (inside the repo)
...
```bash
ros2 run raisebot_tools grasp_server                                  # terminal 3
ros2 run raisebot_labs 06_replay_episode.py --task C --team team07 \
    --hf-user me --episode 0 --spawn                                   # terminal 2
# alias:  ros2 run raisebot_labs 06_* --task C --team team07 --hf-user me --episode 0 --spawn
```
