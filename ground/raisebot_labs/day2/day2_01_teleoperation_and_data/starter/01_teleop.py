...
"""
RaiseBot — Lab 2.1, Script 01 — keyboard teleoperation of the UR5e (+ Husky).
...
Run while `agr-sim ground` is up:
    ...
    ros2 run raisebot_labs 01_teleop.py            # arm only
    ros2 run raisebot_labs 01_teleop.py --task B   # also enable base driving
    # or:  ros2 run raisebot_labs 01_*
"""
...
class Teleop(Node):
    ...
    def publish_base(self, lin: float, ang: float):
        if self.enable_base:
            ...
            print('  (base driving is OFF — restart with:  ros2 run raisebot_labs 01_* --task B)')
...
def main():
    ap = argparse.ArgumentParser(description='RaiseBot keyboard teleop')
