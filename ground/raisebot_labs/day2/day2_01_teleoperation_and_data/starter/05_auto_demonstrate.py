...
"""
RaiseBot — Lab 2.1, Script 05 — AUTO-DEMONSTRATOR ("pick the red tomato").
...
PREREQUISITES (three things must be running):
    ...
    2. the grasp server:  ros2 run raisebot_tools grasp_server
    3. this script:       ros2 run raisebot_labs 05_auto_demonstrate.py --episodes 30
                          (or:  ros2 run raisebot_labs 05_* --episodes 30)
...
from raisebot_tools import gz_utils   # noqa: E402
...
class Demonstrator(Node):
    ...
    def tool_point(self, grasp_offset: float = 0.13):
        ...
        try:
            ...
            return None
        wtb = gz_utils.get_model_world_pose('raisebot_robot', self.world)
        if wtb is None:
...
def main():
    ap = argparse.ArgumentParser(description='RaiseBot auto-demonstrator (pick the red tomato)')
    ...
    ap.add_argument('--data-root', default=str(_d2paths.DATASETS_DIR),
                    help='datasets live INSIDE the repo (RaiseBot/datasets) so everything is versioned together')
    ...
    check_parking(lambda: gz_utils.get_model_world_pose('raisebot_robot', args.world))
