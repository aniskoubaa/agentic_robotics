...
"""
RaiseBot — Lab 2.1, Script 06 — REPLAY a recorded episode into Gazebo.
...
PREREQUISITES:  the sim (agr-sim ground).  For the grasp to reproduce, also:
    ros2 run raisebot_tools grasp_server      # (alias: grasp_d3)

Usage:
    # replay episode 0 of your dataset (motion only):
    ros2 run raisebot_labs 06_replay_episode.py --task C --team team07 --hf-user me --episode 0
    # replay AND recreate the tomatoes so the pick reproduces:
    ros2 run raisebot_labs 06_replay_episode.py --task C --team team07 --hf-user me --episode 0 --spawn
    # (alias: ros2 run raisebot_labs 06_* ...)
"""
...
def main():
    ap = argparse.ArgumentParser(description='RaiseBot episode replayer')
