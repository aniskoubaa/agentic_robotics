...
"""
RaiseBot — Lab 2.1, Script 03 — record demonstrations in LeRobot format.
...
Workflow (two terminals):
    Terminal 1:  ros2 run raisebot_labs 01_teleop.py --task A
    Terminal 2:  ros2 run raisebot_labs 03_record.py --task A --team team07 \
                     --hf-user raiseschool --episodes 30
    Then follow the prompts: ENTER to start an episode, ENTER to stop, keep/discard.
    (or:  ros2 run raisebot_labs 03_* ...)
"""
...
def main():
    ap = argparse.ArgumentParser(description='RaiseBot LeRobot demo recorder')
