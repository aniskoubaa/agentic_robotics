...
|---|--------|-------|------------------------|
| 01 | `01_teleop.py` | `ros2 run raisebot_labs 01_*` | drive the arm+gripper from the keyboard (the publisher pattern, one level below Day 1) |
| 02 | `02_read_streams.py` | `ros2 run raisebot_labs 02_*` | watch `/wrist_camera` + `/joint_states` live — the exact streams the VLA learns from (pre-flight check) |
| 03 | `03_record.py` | `ros2 run raisebot_labs 03_*` | manual recording: fuse the streams into LeRobot episodes while YOU drive |
| 05 | `05_auto_demonstrate.py` | `ros2 run raisebot_labs 05_*` | **hands-free recording**: the expert SCANS (looks above left, then right), picks the red tomato it SEES, side alternating — every grasp verified. Active perception! |
| 06 | `06_replay_episode.py` | `ros2 run raisebot_labs 06_*` | **replay** a recorded episode into the sim — THE data check before training |
| 04 | `04_upload.py` | `ros2 run raisebot_labs 04_*` | *(optional — multi-machine schools only; this edition trains locally)* |
...
Your dataset lands **inside the repo**: `RaiseBot/datasets/me__raise_ripeness_sort_team07`.
