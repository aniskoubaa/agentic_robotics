...
# The RaiseBot Reference Dataset — what we built and how
...
| Location | `RaiseBot/datasets/raiseschool__raise_ripeness_sort_ref` (inside the repo — versioned with the code) |
...
```bash
# sim (headless works!)     ros2 launch raisebot_bringup sim.launch.py headless:=true
# the grasp                 ros2 run raisebot_tools grasp_server
# the demonstrator          ros2 run raisebot_labs 05_* --episodes 50 --team ref
```
...
ds = LeRobotDataset('raiseschool/raise_ripeness_sort_ref',
                    root='<repo>/RaiseBot/datasets/raiseschool__raise_ripeness_sort_ref')
...
ros2 run raisebot_labs 06_* --task C --team ref --episode 0 --spawn
```
...
- **More episodes:** `ros2 run raisebot_labs 05_* --episodes 50 --team ref2`
- **More variety:** add jitter to the two grasp poses in `05_auto_demonstrate.py`
  (`GRASP_LEFT/RIGHT`), or add a third spawn point.
- **A row-conditioned task:** `ros2 run raisebot_labs 05_* --row 2` weaves "in row 2" into the
