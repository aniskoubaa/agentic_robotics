#!/usr/bin/env python3
"""
RaiseBot — Lab 1.2 — the agentic inspector (autonomous mission → field report).
...
The `ros2 run raisebot_labs agent.py` alias starts the servers for you. Manual:

    ros2 run raisebot_tools gripper_server &
    ros2 run raisebot_tools move_to_pose_server &
    ros2 run raisebot_tools navigation_server &
    ros2 run raisebot_tools detector_server &
    ros2 run raisebot_tools inspector_server &
    ros2 run raisebot_labs agent.py              # or:  ros2 run raisebot_labs agent.py
...
def main():
    ...
    BANNER = """
┌─── RaiseBot · Agentic Inspector (autonomous mission) ────┐
