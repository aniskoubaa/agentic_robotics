...
- The five tool servers from Lab 1.1 are running (the `ros2 run raisebot_labs agent.py` alias starts
  them for you)
...
```bash
ros2 run raisebot_labs agent.py
```
...
ros2 run raisebot_labs agent.py          # press TAB after raisebot_labs
```

**C. Direct python3 while editing** (servers must already be up):

```bash
cd ~/ros2_ws/src/agentic_robotics/ground/raisebot_labs/day1/day1_02_agentic_inspector/starter
python3 agent.py
```
