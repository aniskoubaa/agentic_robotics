...
```bash
ros2 run raisebot_labs 01_drive_forward.py     # press TAB after raisebot_labs
ros2 run raisebot_labs 02_read_lidar.py        # Ctrl-C to quit
ros2 run raisebot_labs 03_read_camera.py       # writes /tmp/raise_camera_frame.png
ros2 run raisebot_labs 04_aim_ptz.py           # 14-s PTZ look-around routine
# Scripts 05–09 each need a matching server in another terminal.
# The ros2 run raisebot_labs 05_* … ros2 run raisebot_labs 10_* aliases bundle the server + client for you, e.g.:
ros2 run raisebot_labs 05_*   # starts gripper_server, runs 05_call_gripper.py, cleans up
ros2 run raisebot_labs 10_*   # starts all 5 servers, runs 10_orchestrate.py (the LLM agent)
```

**B. Direct python3 from the source tree** *(useful while you're editing a script)*

```bash
cd ~/ros2_ws/src/agentic_robotics/ground/raisebot_labs/day1/day1_01_ros2_tools_as_functions/starter
