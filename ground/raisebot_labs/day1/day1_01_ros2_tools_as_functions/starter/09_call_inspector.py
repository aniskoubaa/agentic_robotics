#!/usr/bin/env python3
"""
RaiseBot — Lab 1, Script 09 — VLM inspector.
...
WHAT  Send the current camera frame to an LLM with vision (GPT-4o)
      ...
      A live OpenCV window ("RaiseBot - VLM inspector") shows the camera
...
║     raisebot_tools/raisebot_tools/inspector_server.py                   ║
║                                                                           ║
...
Before running this you need:
    ...
    ros2 run raisebot_tools inspector_server   # in another terminal

Then run this client:

    ros2 run raisebot_labs 09_call_inspector.py    # or:  ros2 run raisebot_labs 09_*
"""
    ...
    for key, (label, srv_name) in ACTIONS.items():
        if not clients[key].wait_for_service(timeout_sec=SERVER_TIMEOUT):
            node.get_logger().error(
                ...
                f'    ros2 run raisebot_tools inspector_server\n'
