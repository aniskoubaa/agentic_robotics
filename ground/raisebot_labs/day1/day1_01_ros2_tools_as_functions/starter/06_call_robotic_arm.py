#!/usr/bin/env python3
"""
RaiseBot — Lab 1, Script 06 — move the UR5e robotic arm.
...
║     raisebot_tools/raisebot_tools/move_to_pose_server.py                ║
║                                                                           ║
...
Then run this client:
    ...
    ros2 run raisebot_labs 06_call_robotic_arm.py   # or:  ros2 run raisebot_labs 06_*
"""
    ...
    for key, (label, srv_name) in ACTIONS.items():
        if not clients[key].wait_for_service(timeout_sec=SERVER_TIMEOUT):
            node.get_logger().error(
                ...
                f'    ros2 run raisebot_tools move_to_pose_server'
            )
