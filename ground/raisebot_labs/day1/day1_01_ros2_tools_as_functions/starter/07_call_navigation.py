#!/usr/bin/env python3
"""
RaiseBot — Lab 1, Script 07 — drive the Husky to a named waypoint.
...
║     raisebot_tools/raisebot_tools/navigation_server.py                  ║
║                                                                           ║
...
Then run this client:
    ...
    ros2 run raisebot_labs 07_call_navigation.py    # or:  ros2 run raisebot_labs 07_*
"""
    ...
    for key, (label, srv_name) in ACTIONS.items():
        if not clients[key].wait_for_service(timeout_sec=SERVER_TIMEOUT):
            node.get_logger().error(
                ...
                f'    ros2 run raisebot_tools navigation_server'
            )
