...
#
#     __import__('pkg_resources').require('raisebot-labs==0.1.0')
...
# of develop, so `import raisebot_labs` still works.
    ...
    def _write_simple_wrapper(self, src_path: str) -> None:
        ...
        with open(target, 'w') as f:
            f.write(
                ...
                '# RaiseBot — dev-script wrapper (no pkg_resources, no DeprecationWarning).\n'
...
# Scripts still install by basename into lib/raisebot_labs/, so
# `ros2 run raisebot_labs <script>.py` is unaffected by the nesting.
LABS = [
...
def lab_readme_data_files():
    """Install each lab's README under share/raisebot_labs/<lab>/README.md."""
    files = []
    for lab in LABS:
...
def day2_lib_data_files():
    ...
    the libs under share/raisebot_labs/day2_lib/ and _d2paths falls back to
...
setup(
    ...
    description='RaiseBot lab starter scripts.',
    license='MIT',
