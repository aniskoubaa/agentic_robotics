...
Three minimal drivers for the AgriBot Husky — keyboard, joystick, phone.
...
|---|---|---|
| Keyboard | `ros2 run agribot_teleop teleop_keyboard` | Quick driving from any terminal |
| Joystick | `ros2 launch agribot_teleop teleop_joy.launch.py` | Smooth analog control with a gamepad |
| Phone   | `ros2 run agribot_teleop teleop_phone` | Demos — let an audience tap-drive |

## 1. Keyboard

```
ros2 run agribot_teleop teleop_keyboard
```
...
Code: `agribot_teleop/teleop_keyboard.py` — ~70 lines, no extra deps.
...
```bash
ros2 launch agribot_teleop teleop_joy.launch.py
```
...
```bash
ros2 run agribot_teleop teleop_phone
...
Code:
- `agribot_teleop/teleop_phone.py` — Flask + rclpy bridge (~80 lines)
...
```bash
ros2 run agribot_teleop camera_view
