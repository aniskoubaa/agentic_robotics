...
Three minimal drivers for the RaiseBot Husky — keyboard, joystick, phone.
...
|---|---|---|
| Keyboard | `ros2 run raisebot_teleop teleop_keyboard` | Quick driving from any terminal |
| Joystick | `ros2 launch raisebot_teleop teleop_joy.launch.py` | Smooth analog control with a gamepad |
| Phone   | `ros2 run raisebot_teleop teleop_phone` | Demos — let an audience tap-drive |
...
Code: `raisebot_teleop/teleop_keyboard.py` — ~70 lines, no extra deps.
...
```bash
ros2 launch raisebot_teleop teleop_joy.launch.py
```
...
```bash
ros2 run raisebot_teleop teleop_phone
...
Code:
- `raisebot_teleop/teleop_phone.py` — Flask + rclpy bridge (~80 lines)
