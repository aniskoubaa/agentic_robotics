...
Go under `srv/`. **Adding any custom interface promotes this package from `ament_python` to `ament_cmake`** (rosidl requirement). If we add custom types, the cleaner move is to split a `raisebot_interfaces` package and keep this one Python-only.
